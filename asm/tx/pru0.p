// Transmitter - PRU0 source code.

// Responsible for pulling outgoing data from PRU1 and modulating 
// the bitstream on the output pin (Header 8, Pin 15). On init,
// PRU0 will load the number of packets to modulate on the current
// transmission, and will track its packet count thereafter. The operating
// principle of modulation is straightforward: each bit of the stream is checked,
// and the output pin set to its value. The program then delays for 
// a deterministic amount of time (see include/asm.hp) before setting
// the next bit, effectively implementing our bit-timing. As an implementation
// detail, register 9 (r9) is used for holding the current 32-bit section of
// outgoing data, with new data being shifted in as needed. Upon completing
// an entire packet the loop restarts, pulling another packet from PRU1 until
// the specified number of packets has been reached.
