...
class RealtimeControl {
  ...
  public:
    ...
    uint8_t* peek() {
        return (uint8_t*)_data + _internal_cursor;
    }

    uint8_t* data() {
        return (uint8_t*)_data;
    }
    void setCursor(uint32_t val) {
        _internal_cursor = val;
    }

    uint32_t pruCursor() {
        return *_pru_cursor - 8;
    }
    uint32_t internalCursor() {
